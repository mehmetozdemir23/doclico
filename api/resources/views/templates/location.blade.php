...
<body>
    ...
    <div class="section">
        ...
        <div class="content">
            Type de bien : {{ $bienTypeTexte }}
            @if (!empty($data['bien_superficie']))
                de {{ $data['bien_superficie'] }}
            @endif
            situé {!! nl2br(e($data['bien_adresse'] ?? '')) !!}.
        </div>
    </div>
    ...
    <div class="section">
        ...
        <div class="content">
            Loyer mensuel : {{ $data['loyer_montant'] ?? '' }} euros
            @if (!empty($data['charges_montant']))
                + {{ $data['charges_montant'] }} euros de charges
            @endif
        </div>
        @if (!empty($data['depot_garantie']))
            <div class="content">
                Dépôt de garantie : {{ $data['depot_garantie'] }} euros
            </div>
        @endif
    </div>
    ...
    <div class="section">
        ...
        <div class="content">
            Le présent bail prend effet le {{ $dateDebut }}{{ !empty($data['duree']) ? " pour une durée de {$data['duree']}" : '' }}.
        </div>
    </div>

    <div class="content">
        Le locataire s'engage à payer le loyer à la date convenue et à user du logement en bon père de famille.
    </div>
